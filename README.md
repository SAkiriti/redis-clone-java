...
Built on Windows, so the commands above are `mvn.cmd` rather than `mvn` locally.

Running the server on Windows with `redis-cli` in WSL: WSL can't reach Windows on localhost, so connect with `redis-cli -h <windows-ip>`. Find the IP with `ip route show | grep default`; it changes after a reboot.
